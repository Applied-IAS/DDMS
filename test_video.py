...
for data_config in config.data_configs:
    ...
    for pred_mode in config.pred_modes:
        for transform_mode in config.transform_modes:
            ...
            # load the pretrained model, manually slect the model
            loaded_param = torch.load(
                str(f"{results_folder}/{model_name}_{9}.pt"),
            ...
            for k, batch in enumerate(val_data):
                ...
                for i, b in enumerate(
                    ...
                    if not os.path.isdir(f"../results/evaluate/truth/{model_name}"):
                        os.makedirs(f"../results/evaluate/truth/{model_name}")
                    Parallel(n_jobs=4)(
                        delayed(save_image)(f, f"../results/evaluate/truth/{model_name}/{k}-{i}-{j}.png")
                        for j, f in enumerate(b.cpu())
                    )
                    write_video(
                        f"../results/evaluate/truth/{model_name}/{k}-{i}.mp4",
                ...
                # print('this is a test for sampled')
                # print(sampled.shape)
                sampled = sampled[:, config.N_CONTEXT:, :, :,:]
                # print(sampled.shape)
                # exit(0)
                # for i, b in enumerate(sampled.clamp(0, 1)):
                #     if not os.path.isdir(f"evaluate/generated/{model_name}"):
                #         os.makedirs(f"evaluate/generated/{model_name}")
                #     Parallel(n_jobs=4)(
                ...
                for i, b in enumerate(sampled.clamp(0, 1)):
                    print(count)
                    if not os.path.isdir(f"../results/evaluate/generated/{model_name}"):
                        os.makedirs(f"../results/evaluate/generated/{model_name}")
                    if not os.path.isdir(f"../results/evaluate/generated/{model_name}/video/" + str(count)):
                        os.makedirs(f"../results/evaluate/generated/{model_name}/video/" + str(count))
                    if not os.path.isdir(f"../results/evaluate/generated/{model_name}/pred/" + str(count)):
                        os.makedirs(f"../results/evaluate/generated/{model_name}/pred/" + str(count))
                    Parallel(n_jobs=4)(
                        delayed(save_image)(f, f"../results/evaluate/generated/{model_name}/pred/{count}/{j}.png")
                        for j, f in enumerate(b.cpu())
                    )
                    write_video(
                        f"../results/evaluate/generated/{model_name}/video/{count}/video.mp4",
                        torch.round(255 * b.permute(0, 2, 3, 1)).expand(-1,-1,-1,3).cpu(),
                        fps=4,
                    )
                    count += 1